...
class DistanceTransformer
{
    public:
        ...
        static DistanceTransformation calculateDistanceTransformation(const std::vector<OrientedGradient>& inputOG, int width, int height);
        static std::vector<float> calculateDistanceTransformation(const std::vector<bool>& binaryImg, int width, int height);
