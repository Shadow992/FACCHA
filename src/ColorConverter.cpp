...
std::vector<ColorXYZ> ImageFramework::convertRGBToXYZ(Image* img)
{
    ...
    {
        float valR = ( (float)imgRef.imgData[i].color.red / 255.0 );        //R from 0 to 255
        float valG = ( (float)imgRef.imgData[i].color.green / 255.0 );        //G from 0 to 255
        float valB = ( (float)imgRef.imgData[i].color.blue / 255.0 );        //B from 0 to 255
...
Image* ImageFramework::convertXYZToRGB(std::vector<ColorXYZ>& xyzImg, int width, int height)
{
    ...
    if(img==nullptr || convertXYZToRGB(xyzImg,img)==false)
    {
        return nullptr;
    }
