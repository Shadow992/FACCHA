...
void DistanceTransformer::calculateDistanceTransformation1D(const std::vector<float>& f, std::vector<float>& d)
{
    ...
    for (int q = 1; q < n; q++)
    {
        ...
        float s = (part1 - (f[parabolaLocation[idxRightmostParabola]]
                               + parabolaLocation[idxRightmostParabola] * parabolaLocation[idxRightmostParabola]))
            / (2.0 * q - 2.0 * parabolaLocation[idxRightmostParabola]);
        while (s <= boundaryLocation[idxRightmostParabola])
        {
            idxRightmostParabola--;
            s = (part1 - (f[parabolaLocation[idxRightmostParabola]]
                             + parabolaLocation[idxRightmostParabola] * parabolaLocation[idxRightmostParabola]))
                / (2.0 * q - 2.0 * parabolaLocation[idxRightmostParabola]);
        }
        idxRightmostParabola++;
...
DistanceTransformation DistanceTransformer::calculateDistanceTransformation(
...
std::vector<float> DistanceTransformer::calculateDistanceTransformation(
