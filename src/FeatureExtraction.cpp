...
std::vector<ImageLBP> ImageFramework::calculateLBPOfImage(Image* img, int radius, int threshold, int regionMeanThreshold, bool fillHoles)
{
    Image& imgRef=*img;
        ...
        for(int x=radius; x<newWidth; x++)
        {
            ...
            for(int colorIdx=0; colorIdx<3; colorIdx++)
            {
                ...
                for(unsigned int i=0; i<extractedPixel.size(); i++)
                {
                    ...
                    const int usedColorExtracted[]= {extractedPixel[i]->color.red, extractedPixel[i]->color.green, extractedPixel[i]->color.blue};
                ...
                if(countOfAlternations==2)
                {
                    ...
                    output[colorIdx].gradientMagnitudeSquare=output[colorIdx].gradientX*output[colorIdx].gradientX+output[colorIdx].gradientY*output[colorIdx].gradientY;
            ...
            if(output[bestColor].length!=-1)
            {
                ...
                if(regionMeanThreshold>0 && x > regionRadius && y > regionRadius && x < imgRef.width-regionRadius && y < imgRef.height-regionRadius)
                {
