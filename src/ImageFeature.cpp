...
void ImageFramework::invertImage(Mat& img)
{
    ...
    {
        img.forEach<float>([](float& p, const int* position) -> void { p = 1.0 - p; });
    }
}
...
// http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.167.1500&rep=rep1&type=pdf
// + added global rarity voting
Mat ImageFramework::getSaliencyMap(
    Mat& img, int smallWindowSize, int scaleStep, int startingScale, int endingScale, int randomOtherRegions)
{
    ...
    for (int scales = startingScale; scales <= endingScale; scales += scaleStep)
    {
        ...
        for (int i = 0; i < randomOtherRegions; i++)
        {
            ...
            const Vec3i avgValRan
                = getIntegralImageMean(integralImg, xStartRandom, xEndRandom, yStartRandom, yEndRandom);
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                const Vec3i avgValSmall
                    = getIntegralImageMean(integralImg, xStartSmall, xEndSmall, yStartSmall, yEndSmall);
