...
Pixel* ImageFramework::searchForImage(Image* img, int startX, int startY, const Image* searchImage, unsigned char variance)
{
    Image& imgRef=*img;
    ...
    for (int y=startY; y < imgRef.height; y++)
    {
        for (int x=startX; x < imgRef.width; x++)
        {
            ...
            for (int searchY=0; searchY <searchImageHeight; searchY++)
            {
                ...
                {
                    if(x + searchX<imgRef.width && y + searchY<imgRef.height && imgRef.getPixel(x + searchX, y + searchY)->colorEquals(searchImageRef.getPixel(searchX, searchY), variance)==false)
                    {
                        match=false;
                        break;
                    }
...
Pixel* ImageFramework::searchForImageNextToPosition(Image* img, int x, int y, const Image* searchImage, unsigned char variance)
{
    Image& imgRef=*img;
    ...
    for(int radius=0;; radius++)
    {
        ...
        for (int nearestX=x-radius; nearestX <= x+radius; nearestX++)
        {
            if(y-radius>=0)
            {
                if(nearestX>=0 && nearestX<imgRef.width && equalsImage(img, nearestX,y-radius,searchImage,variance)==true)
                {
        ...
        for (int nearestY=y-radius; nearestY <= y+radius; nearestY++)
        {
            ...
            {
                if(nearestY>=0 && nearestY<imgRef.height && equalsImage(img, x+radius,nearestY,searchImage,variance)==true)
                {
...
Pixel* ImageFramework::searchForPixel(Image* img, int startX, int startY, const Color& searchPixel, unsigned char variance)
{
    return searchForPixel(img, startX, startY, searchPixel.red, searchPixel.green, searchPixel.blue,variance);
}

Pixel* ImageFramework::searchForPixel(Image* img, int startX, int startY, unsigned char red,unsigned char green,unsigned char blue, unsigned char variance)
{
    Image& imgRef=*img;
...
Pixel* ImageFramework::searchPixelNextToPosition(Image* img, int x, int y, const Color& searchPixel, unsigned char variance)
{
    return searchPixelNextToPosition(img, x, y, searchPixel.red, searchPixel.green, searchPixel.blue, variance);
}

Pixel* ImageFramework::searchPixelNextToPosition(Image* img, int x, int y, unsigned char red, unsigned char green, unsigned char blue, unsigned char variance)
{
    Image& imgRef=*img;
    ...
    for(int radius=0;; radius++)
    {
        ...
        for (int nearestX=x-radius; nearestX <= x+radius; nearestX++)
        {
            if(y-radius>=0)
            {
                if(nearestX>=0 && nearestX<imgRef.width && imgRef.getPixel(nearestX,y-radius)->colorEquals(red,green,blue,variance))
                {
                    return imgRef.getPixel(nearestX,y-radius);
                }
            }
        ...
        for (int nearestY=y-radius; nearestY <= y+radius; nearestY++)
        {
            if(x+radius<imgRef.width)
            {
                if(nearestY>=0 && nearestY<imgRef.height && imgRef.getPixel(x+radius,nearestY)->colorEquals(red,green,blue,variance))
                {
                    return imgRef.getPixel(x+radius,nearestY);
                }
            }
...
float ImageFramework::checkHistogramDifferences(const std::vector<float>& histogram, const std::vector<float>& histogram2)
{
...
float ImageFramework::findSimilarImageHillClimbing(Image* img, int startX, int startY,int w, int h,Image* checkImage, int accuracy, float overlap, int &bestX, int &bestY)
{
    Image& imgRef=*img;
...
void ImageFramework::findBetterSolution(Image* img, std::vector<float>& histogramSearch, int x, int y,int w, int h,Image* checkImage, int accuracy, int &bestX, int &bestY, float& fitness)
{
    Image& imgRef=*img;
    Image& checkImageRef=*checkImage;
    ...
    while(foundBetter==true)
    {
        ...
        for (int nearestX=x-radius; nearestX <= x+radius; nearestX++)
        {
            if(y-radius>=0)
            {
                ...
                {
                    auto histogramSrc=calcHistogram(checkImage, accuracy,nearestX,y-radius,widthToCheck,heightToCheck);
                    currFitness=checkHistogramDifferences(histogramSearch,histogramSrc);
        ...
        for (int nearestY=y-radius; nearestY <= y+radius; nearestY++)
        {
            if(x+radius<w)
            {
                ...
                {
                    auto histogramSrc=calcHistogram(checkImage, accuracy,x+radius,nearestY,widthToCheck,heightToCheck);
                    currFitness=checkHistogramDifferences(histogramSearch,histogramSrc);
                    if(currFitness<fitness)
                    {
                        fitness=currFitness;
