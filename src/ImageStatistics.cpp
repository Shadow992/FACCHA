...
bool ImageFramework::calculateSecondOrderDerivatives(Image* img, int x, int y, int& Ixx, int& Ixy, int& Iyy)
{
    ...
    const int xxFilter[]={1,0,-2,0,1,
                  4,0,-8,0,4,
                  6,0,-12,0,6,
                  4,0,-8,0,4,
                  1,0,-2,0,1};
    const int yyFilter[]={1,4,6,4,1,
                  0,0,0,0,0,
                  -2,-8,-12,-8,-2,
                  0,0,0,0,0,
                  1,4,6,4,1};
    const int xyFilter[]={1,2,0,-2,-1,
                  2,4,0,-4,-2,
                  0,0,0,0,0,
                  -2,-4,0,4,2,
                  -1,-2,0,2,1};
...
int ImageFramework::getRegionDifference(Image* img, int x, int y, int radius)
{
    ...
    const int meanDifLeftRight=std::abs(meanLeftRegionR-meanRightRegionR)+std::abs(meanLeftRegionG-meanRightRegionG)+std::abs(meanLeftRegionB-meanRightRegionB);
    const int meanDifUpperLower=std::abs(meanUpperRegionR-meanLowerRegionR)+std::abs(meanUpperRegionG-meanLowerRegionG)+std::abs(meanUpperRegionB-meanLowerRegionB);

    return std::max(meanDifLeftRight,meanDifUpperLower);
}

int ImageFramework::getSetPixelCountInLine(Image* img, const std::vector<bool>& binPixel,int& totalPixelCountInLine,int xStart, int yStart, int xEnd, int yEnd, int widthArea)
{
...
int ImageFramework::getPointSymmetricDifferences(Image* img, int x, int y, int radius, int mean)
{
    ...
    for(int yStep=-radius; yStep<=radius; yStep++)
    {
        for(int xStep=-radius; xStep<0; xStep++)
        {
            ...
            if ((currPixelLeft->color.red > mean && currPixelRight->color.red < mean) || (currPixelLeft->color.red < mean && currPixelRight->color.red > mean))
            {
                differences++;
            }
        }
...
int ImageFramework::countPixelAlternationsRingMorphology(const std::vector<Pixel*>& pixelData, int mean,int& countBlackArea1 ,int& countBlackArea2,int& countWhiteArea1 ,int& countWhiteArea2)
{
    Pixel* currPixel;
