...
int IntegralImage::getRegionDifference(int x, int y, int radius, int& direction)
{
    ...
    const int meanDifLeftRight
        = std::abs(leftMeanR - rightMeanR) + std::abs(leftMeanG - rightMeanG) + std::abs(leftMeanB - rightMeanB);
    const int meanDifUpperLower
        = std::abs(topMeanR - botMeanR) + std::abs(topMeanG - botMeanG) + std::abs(topMeanB - botMeanB);
